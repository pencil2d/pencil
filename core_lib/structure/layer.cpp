...
/**
 * @brief Layer::hasReachedEndOfTrack
 * @param frameNumber
 * @return true is end of track has been reached, otherwise false
 * Use this to check whether we're reached the end of a track.
 * Only valid for sound.
 */
bool Layer::hasReachedEndOfTrack(int frameNumber)
{
    auto keyFrame = getLastKeyFrameAtPosition(frameNumber);
...
KeyFrame* savedFrame;
bool Layer::isTrackOverlapping(int currentFrame)
{
    qDebug() << "mSaveFrame: " << mSaveFrame;
    auto keyFrame = getLastKeyFrameAtPosition(currentFrame);
    if ( keyFrame != nullptr && !mSaveFrame ){
        savedFrame = keyFrame;
        mSaveFrame = true;
        qDebug() << "///////////////// saved frame";
    }

    auto prevFrame = savedFrame;
    if (keyFrame != nullptr && prevFrame != nullptr)
    {

        if (currentFrame >= prevFrame->pos() + prevFrame->length())
        {
            mSaveFrame = false;
        }
        if (keyFrame->pos() + keyFrame->length() < prevFrame->pos() + prevFrame->length())
        {
            qDebug() << "overlapping here!";
            qDebug() << "c: " << currentFrame;
            qDebug() << "p: " << prevFrame->pos()+prevFrame->length();
            if (currentFrame >= prevFrame->pos() + prevFrame->length())
            {
                qDebug() << "entered";
                return false;
...
//        qDebug() << "current: " << keyFrame->pos() + keyFrame->length() << " \n";
//        qDebug() << "previous: " << prevFrame->pos() + prevFrame->length();
    }
    return false;
}
