...
void BucketTool::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (event->button() == Qt::LeftButton)
    {
        ...
        {
            BitmapImage *sourceImage = ((LayerBitmap *)layer)->getLastBitmapImageAtFrame(m_pEditor->layerManager()->currentFramePosition(), 0);
            ...
            BitmapImage *targetImage = ((LayerBitmap *)targetLayer)->getLastBitmapImageAtFrame(m_pEditor->layerManager()->currentFramePosition(), 0);
            ...
            m_pScribbleArea->setModified(layerNumber, m_pEditor->layerManager()->currentFramePosition());
            m_pScribbleArea->setAllDirty();
        }
