...
TEST_CASE("ViewManager: working with camera layers")
{
    ...
    SECTION("Camera Layer with 2 keys")
    {
        ...
        // 2nd key at frame 10
        LayerCamera* layerCam = editor->object()->addNewCameraLayer();
        // following line needed since new frame gets translation of previous frame
        // with the new interpolation feature (David 2021)
        layerCam->getCameraAtFrame(1)->translate(QPointF(100, 0));
        layerCam->addKeyFrame(10, new Camera(QPointF(0, 0), 0, 1));
